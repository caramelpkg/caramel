...
class downloader(object):
    ...
    def download_tropomi_l2(self,product:str,output_fld:Path,maxpage=30,username="s5pguest",password="s5pguest"):
        ...
        if product == 1: product_name = "%5F%5FNO2%5F%5F%5F"
        if product == 2: product_name = "%5F%5FHCHO%5F%5F"
        if product == 3: product_name = "%5F%5FCH4%5F%5F%5F"
        if product == 4: product_name = "%5F%5FCO%5F%5F%5F%5F"
        # loop over the pages
        for page in range(0, maxpage):
            ...
            searcher += f"{self.latll:.4f}" + "," + f"{self.lonur:.4f}" +"%20" + f"{self.latur:.4f}" 
            searcher +=  "," + f"{self.lonll:.4f}" + "%20" + f"{self.latur:.4f}" +","
            searcher += f"{self.lonll:.4f}" + "%20" f"{self.latll:.4f}" +")))%22%20AND%20(%20beginPosition:"
            ...
            with open('temp/tropomi.xml', 'r') as f:
                ...
                for fname in list_file:
                    ...
                    cmd += " -P" + (output_fld.stem) + "/"
                    if not os.path.exists(output_fld.stem): os.makedirs(output_fld.stem)
                    os.system(cmd)
