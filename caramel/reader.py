...
def tropomi_reader_no2_wrapper(fname:str,interpolation_flag = True) -> satellite:
    ...
    tropomi_no2.time = datetime.datetime(2010, 1, 1) + datetime.timedelta(seconds=int(time))
    ...
    # read quality flag
    tropomi_no2.qa = _read_group_nc(fname,1,'PRODUCT','qa_value').astype('float32')
    ...
    trop_layer = _read_group_nc(fname,1,'PRODUCT','tm5_tropopause_layer_index')
...
def tropomi_reader(product_dir:str,num_job = 1):
    ...
    outputs = Parallel(n_jobs=num_job)(delayed(tropomi_reader_no2_wrapper)(L2_files[k]) for k in range(len(L2_files)))
...
class readers(object):
    ...
    def read_satellite_data(self, num_job = 1):
        if self.satellite_product_num == 1: #TROPOMI NO2
           tropomi_reader(self.satellite_product_dir.stem, num_job = num_job)
        
# testing
if __name__ == "__main__":

    reader_obj = readers()
    reader_obj.add_satellite_data(1,Path('download_bucket/'))
