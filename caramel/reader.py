...
class readers(object):
    def __init__(self,product_name:int,product_dir:Path) -> None:
        '''
        ...
        if product_name == 1: pass

    def tropomi_reader_no2(self,num_job = 1) -> tropomi:
        '''
        ...
        def reader_wrapper(fname):
            ...
            tropomi.time = datetime.datetime(2010, 1, 1) + datetime.timedelta(seconds=int(time))
            # read lat/lon at corners
            tropomi.latitude_corner = np.array(_read_group_nc(fname,1,'PRODUCT','latitude'))
            tropomi.longitude_corner = np.array(_read_group_nc(fname,1,'PRODUCT','longitude'))
            # read total amf
            ...
            tropomi.vcd = vcd*6.02214*1e19*1e-15
            # read quality falg
            tropomi.qa = _read_group_nc(fname,1,'PRODUCT','qa_value')
            ...
            tropomi.pressure_mid = p_mid
            tropomi.averaging_kernels = SWs
            # read the precision
            tropomi.uncertainty = np.array(_read_group_nc(fname,1,'PRODUCT','nitrogendioxide_tropospheric_column_precision'))
            # return
            return tropomi

        # find L2 files first
        L2_files = sorted(glob.glob(self.product_dir + "*.nc"))
